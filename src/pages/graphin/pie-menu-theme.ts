  ...
  pieMenu: {
    container: css`
      ...
      position: ${({ centerX, centerY }) =>
        centerX || centerY ? 'absolute' : 'relative'};
    ...
    item: css`
      ...
      position: absolute;
      transform: rotate(
          ${({ startAngle, endAngle }) => startAngle + endAngle}deg
        )
        skew(${({ skew }) => skew}deg);
      transform-origin: bottom right;
      overflow: hidden;
      //border: 2px solid radial-gradient(transparent 10 #fff 100});
    `,
    center: css`
      position: absolute;
      border-radius: 50%;
  ...
  slice: {
    container: css`
      ...
      background: radial-gradient(
        transparent
          ${({ centerRadius }) => `${centerRadius}, white ${centerRadius}`}
      );
      ...
      &:hover {
        ...
        background: radial-gradient(
          transparent
            ${({ centerRadius }) => `${centerRadius},  #fff ${centerRadius}`}
        );
      }
    `,
